...
namespace Holy {
    ...
    // Colors of 0 to 7
    constexpr unsigned int color_number[] = { 0xC0C0C0, 0xFF0000, 0x008000,
                                              0x0000FF, 0x800000, 0x000080,
                                              0x808000, 0x000000 };
    ...
    // Helper function that does something for every neighbor of a specific
    // block in 8 directions p - The coordinate of the block fn -- the function
    // to be called. fn should be invocable with fn(Coord)
    ...
    // Helper function that does something for every neighbor of a specific
    // block in 4 directions p - The coordinate of the block fn -- the function
    // to be called. fn should be invocable with fn(Coord)
    template <typename Fn>
    ...
    public:
        ...
        Butterfly(HWND hwnd = NULL) {
            ...
            mBitmap = ::CreateCompatibleBitmap(
                ...
                mScreenWidth,
                mScreenHeight);
            mOldBitmap = static_cast<HBITMAP>(
        ...
        int read_block(int x, int y) {
            ...
            for (int d = 0; d < kDeltaPairs; d++) {
                ...
                COLORREF color = ::GetPixel(mMemoryDC, left_top.x, left_top.y);
    ...
    private:
        ...
        static void search_recurse(
            Coord currp,
            Checklist& vis,
            std::vector<HOMO>& output,
            const GameData& game_data) {
            ...
            // It doesn't matter if the current one is a number, because we
            // share the vis with find_exterior()
            for_each_nei4(currp, [&](Coord nextp) {
                if (!vis[hash_point(nextp)])
        ...
        static void
            mark_empty(Coord currp, Checklist& vis, GameData& game) noexcept {
    ...
    public:
        ...
        static std::vector<HOMO>
            search(Coord currp, const GameData& game_data) {
    ...
    bool roundup(
        ...
        for (auto p : homo) {
            ...
            if (block.elabel == block.vacant_nei && block.elabel) {
                ...
                // FIXME: Debug code
                std::cerr << "Reporting error on " << p.x << " " << p.y
                          << std::endl;
                return false;
            }
        }
    ...
    // Applies roundup on all blocks with number, otherwise same as the one
    // above
